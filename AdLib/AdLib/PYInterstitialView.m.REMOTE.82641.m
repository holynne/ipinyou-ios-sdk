...
+ (void)initialize
{
    ...
    dispatch_async(dispatch_get_current_queue(), ^{
        ...
        BOOL set = NO;
        NSInteger appListRecycle = 7;
        NSDate *now = [NSDate date];
        NSString *recieveUrl = @"http://stats.ipinyou.com/mcvt";
        ...
        if(data != nil)
        {
            ...
            if(dictionary != nil)
            {
                ...
                {
                    set = [[dictionary objectForKey:@"showCache"] boolValue];
                }
                if ([dictionary objectForKey:@"appListSendCycle"])
                {
                    appListRecycle = [[dictionary objectForKey:@"appListSendCycle"] integerValue];
                }
                if ([dictionary objectForKey:@"appListRecieveUrl"])
                {
                    recieveUrl =[dictionary objectForKey:@"appListRecieveUrl"];
                }
            }
        }
        if ([[NSUserDefaults standardUserDefaults] objectForKey:@"appListLastSend"]) {
            ...
            NSDate *interval = [now dateByAddingTimeInterval:-appListRecycle*24*60*60];
            NSComparisonResult result = [interval compare:lastDate];
            if (result == NSOrderedDescending) {
                
                PYConversion *conversion = [PYConversion defaultManager];
                [conversion sendAppList: recieveUrl];
        ...
        [[NSUserDefaults standardUserDefaults] setBool:set forKey:@"showCache"];
